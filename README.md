...
生成的项目存放在本机工作区中。当前版本固定使用 `nocodeBuilderProjects` 工作区、`nocodeBuilder` 应用存储目录和 `nocodeBuilder` 凭据服务名；旧版本目录和凭据不会再被自动读取。
...
普通 Agent 文件工具不能写入 `.nocodebuilder/**`。SiteSpec、source map、运行记录、事件、报告和 artifacts 只能通过 Tauri 命令更新。

### Supabase 与 Vercel

Supabase 配置保存在项目 `.env` 中：`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` 用于生成的应用，`SUPABASE_SECRET_KEY` 用于数据库面板读取和写入行数据，`SUPABASE_DB_URL` 用于创建表和修改 schema。进行 schema 变更时建议使用 Supabase Connection Pooler 的 Session mode。
...
- Agent 文件工具不能写入 `.nocodebuilder`、`.env`、`node_modules`、构建输出或项目外路径。
- 命令执行限制在项目命令白名单内。
- 生产部署不会自动触发。
- 写文件前会阻止疑似 secret 内容。
- 取消请求后，迟到的模型结果会被忽略。
...
Generated projects live in a local workspace. Current builds use the `nocodeBuilderProjects` workspace directory, the `nocodeBuilder` app storage directory, and the `nocodeBuilder` credential service name; old directories and credentials are no longer read automatically.
...
Normal Agent file tools cannot write `.nocodebuilder/**`. SiteSpec, source maps, runs, events, reports, and artifacts are updated only through Tauri commands.

### Supabase and Vercel

Supabase settings are stored in the project `.env`: `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` is for generated apps, `SUPABASE_SECRET_KEY` is for the database panel, and `SUPABASE_DB_URL` is used for schema changes. For schema changes, Supabase Connection Pooler Session mode is recommended.
...
- Agent file tools cannot write `.nocodebuilder`, `.env`, `node_modules`, build output, or paths outside the project.
